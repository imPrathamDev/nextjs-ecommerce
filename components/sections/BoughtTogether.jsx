...
function BoughtTogether({ product }) {
    ...
    return (
        <div className='my-6 p-4 rounded-md bg-white'>
            <h3 className='text-2xl font-medium font-Cinzel text-primary-black'>Frequently Bought Together</h3>
            <div className='my-4 grid grid-cols-2 xl:grid-cols-3 lg:grid-cols-3 justify-center items-center gap-2'>
                {
                    product.map(item => (
            ...
            <div className='text-center'>
                <p className='text-lg text-primary-black font-medium font-Cinzel'>Total ₹{product.reduce((acc, curr) => acc + curr?._id?.discPrice, 0)} <span className='text-base line-through'>₹{product.reduce((acc, curr) => acc + curr?._id?.price, 0)}</span></p>
