                    ...
                    <dl className="flex-1 grid grid-cols-2 gap-x-6 text-sm sm:col-span-3 sm:grid-cols-3 lg:col-span-2">
                      ...
                      <div className="hidden sm:block">
                        ...
                        <dd className="mt-1 text-gray-500">
                          <time dateTime={order.createdAt}>{moment(order.createdAt).format("MMM Do YY")}</time>
                        </dd>
                      </div>
                      <div>
                        <dt className="font-semibold text-primary-black">Total amount</dt>
                        <dd className="mt-1 font-medium text-primary-black">₹{order.amount}</dd>
                      </div>
                    </dl>
                      ...
                      <li key={product._id._id} className="p-4 sm:p-6">
                        <div className="flex items-center sm:items-start">
                          ...
                          <div className="flex-1 ml-6 text-sm">
                            <div className="font-medium text-primary-black sm:flex sm:justify-between">
                              <Link href={`/shop/${product._id?.slug}`}><a><h5 className="text-lg font-medium font-Cinzel hover:underline hover:text-primary transition-all">{product._id?.title}</h5></a></Link>
                              <p className="mt-2 sm:mt-0"><span className="text-gray-600">Qty: {product.qty}</span>  ₹{product._id?.discPrice}</p>
                            </div>
                            <p className="hidden text-gray-500 sm:block sm:mt-2">{product._id?.shortDesc}</p>
                          </div>
                        </div>
                        ...
                        <div className="mt-6 sm:flex sm:justify-between items-center">
                          ...
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            ...
                            { order.status === 'Pending' ? 'Order is pending' : order.status === 'Shipped' ? 'Order is on the way' : order.status === 'Complete' ? 'Order is delivered' : order.status === 'cancelled' ? 'Order was Cancelled' : order.status === 'Return' ? 'Order Return' : 'No Info'}
                            </p>
                          </div>
