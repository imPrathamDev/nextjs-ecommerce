...
const ReviewModel = ({ isOpen, setIsOpen, product }) => {
  ...
  const handlerSubmit = () => {
    fetch(`${process.env.NEXT_PUBLIC_HOST}/api/reviews`, {
      method: "POST",
      body: JSON.stringify({
        product: product?._id,
      ...
      .then((data) => {
        if (data?.success) {
          ...
          setIsOpen((prev) => !prev);
          setToast({ show: true, msg: "Review Added", error: false });
        } else {
          setToast({ show: true, msg: data?.error, error: true });
        }
      });
  };
  ...
  return (
    <>
      <Toast setShowToast={setToast} showToast={toast} />
              ...
              <Dialog.Panel className="w-full max-w-lg transform overflow-hidden bg-primary-white ring-2 ring-primary p-6 text-left align-middle shadow-xl transition-all">
                ...
                <div className="mx-2 my-4">
                  <div className="flex items-center gap-1">
                    ...
                    />
                    <h4 className="text-lg my-auto font-medium font-Cinzel">
                      {review?.rating}
                    </h4>
                  </div>
                  <input
                  ...
                  <button
                    ...
                    className="ml-auto w-full text-end text-lg my-2 font-semibold font-Cinzel text-primary hover:underline"
                  >
                    Submit Review
                  </button>
                </div>
