    ...
    const handlerSubmit = async (e) => {
        ...
        if (email.length > 0) {
            if (password.length > 0) {
                ...
                //     // })
                //     // Cookies.set('jwtToken', JSON.stringify(response.token));
                //     setTimeout(() => router.push('http://localhost:3000/dashboard/my-account/'), 1000);
                // } else {
                //     toast.error(response.error == 'Invalid Password' ? 'Invalid Password' : response.error == 'Invalid Email' ? 'Invalid Email' : response.error == 'No User Found' ? 'No User Found' : 'Oops Error!', {
                //         position: "bottom-left",
                //         autoClose: 2000,
                //         hideProgressBar: false,
                //         closeOnClick: true,
                ...
                try {
                    ...
                    if (result.error) {
                        toast.error(result.error.response && result.error.response.data && result.error.response.data.message ? result.error.response.data.message : result.error.message, {
                            position: "bottom-left",
                            autoClose: 2000,
                            hideProgressBar: false,
                            closeOnClick: true,
                    ...
                    toast.error(error.response && error.response.data && error.response.data.message ? error.response.data.message : error.message, {
                        position: "bottom-left",
                        autoClose: 2000,
                        hideProgressBar: false,
                        closeOnClick: true,
                            ...
                            <div className="mt-8">
                                <form method="POST">
                                    <div>
                                        ...
                                        <input type="email" name="email" id="email" placeholder="example@example.com" onChange={handlerChange} value={email} className="block w-full px-4 py-2 mt-2 text-gray-700 placeholder-gray-400 bg-white border-2 border-primary-black focus:border-primary-semi-light dark:focus:border-primary-semi-light focus:ring-primary-semi-light focus:outline-none focus:ring focus:ring-opacity-40" />
                                    </div>
                                    ...
                                    <div className="mt-6">
                                        ...
                                        <input type="password" name="password" id="password" placeholder="Your Password" onChange={handlerChange} value={password} className="block w-full px-4 py-2 mt-2 text-gray-700 placeholder-gray-400 bg-white border-2 border-primary-black focus:border-primary-semi-light  focus:ring-primary-semi-light focus:outline-none focus:ring focus:ring-opacity-40" />
                                    </div>

                                    <button onClick={handlerSubmit} className="mt-6 w-full text-center">
