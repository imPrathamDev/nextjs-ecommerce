...
export async function getServerSideProps(ctx) {
  ...
  if (user) {
    ...
    if (count > 0) {
      myOrders = await Order.find({ userId: user._id })
        ...
        <div className="py-8 sm:py-16">
          <div className="max-w-[90rem] mx-auto sm:px-2 lg:px-8">
            <div className="max-w-2xl mx-auto px-4 lg:max-w-4xl lg:px-0">
              <h1 className="text-2xl font-extrabold font-Cinzel tracking-tight text-primary-black sm:text-3xl">
                Order history
              </h1>
          ...
          {!myOrders.length > 0 && (
            <NotFound message={"Zero orders found ^_^"} />
          )}
