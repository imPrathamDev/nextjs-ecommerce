...
export async function getStaticProps() {
  const getCollections = await fetch('http://localhost:3000/api/collections/getCollections').then((res) => res.json())
  return {
    props: {
...
const CollectionPage = ({ collections }) => {
  const data = collections.filter(c => c.placeholder.toLowerCase() !== 'NA'.toLowerCase())
  return (
    <section className='flex flex-col justify-center'>
      <HeroHeader title="Explore Our Collections" description="Check our collections you will see we provide many diffrent colllections of jewelleries." image={banner} />
