  ...
  switch (action.type) {
    case "ADD_TO_CART":
      Cookies.set(
        ...
        JSON.stringify([
          ...state.cartItems,
          { _id: action.payload._id, qty: 1 },
        ])
      );
      return {
        ...state,
        cartItems: [...state.cartItems, { _id: action.payload._id, qty: 1 }],
      };
    case "REMOVE_FROM_CART":
      Cookies.set(
        "cartItemsData",
        JSON.stringify(
          state.cartItems.filter((c) => c._id !== action.payload._id)
        )
      );
      return {
        ...state,
    ...
    case "ADD_TO_CART_BULK":
      ...
      action.payload.map((item, index) => {
        if (item._id.availableQty > 0) {
          if (
            !state.cartItems.some((checkItem) => checkItem._id == item._id._id)
          ) {
